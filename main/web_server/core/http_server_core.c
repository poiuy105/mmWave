...
esp_err_t http_server_core_start(http_server_t *server)
{
    ...
    // Important: Keep timeout long enough for WebSocket heartbeat (30s interval)
    // Setting too short causes code=1005 (No Status Received) disconnections
    config.recv_wait_timeout = 60;  // 60秒接收超时，支持 WebSocket 心跳
    config.send_wait_timeout = 5;   // 5秒发送超时，检测死连接
