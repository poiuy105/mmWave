...
bool server_config_validate(const server_config_t *config)
{
    ...
    // 栈大小检�?    if (config->http_stack_size < 4096) {
        ESP_LOGE(TAG, "HTTP stack size too small: %u", config->http_stack_size);
        return false;
    }

    // WebSocket 客户端数检�?    if (config->ws_max_clients == 0 || config->ws_max_clients > 16) {
        ESP_LOGE(TAG, "Invalid WS max clients: %d", config->ws_max_clients);
        return false;
    }

    // 超时值检�?    if (config->ws_client_timeout < 10) {
        ESP_LOGE(TAG, "WS client timeout too small: %d", config->ws_client_timeout);
        return false;
    }

    // 消息大小检�?    if (config->ws_max_msg_size < 256 || config->ws_max_msg_size > 65536) {
