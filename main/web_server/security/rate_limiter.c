...
typedef struct {
    ...
    uint32_t window_start;      // 系统启动后的毫秒�?    bool blocked;
    uint32_t block_until;       // 阻塞截止时间
    bool used;                   // 条目是否在使�?} rate_limit_entry_t;
...
bool rate_limiter_check(const char *client_ip)
{
    ...
    // 检查时间窗�?    uint32_t elapsed = now - entry->window_start;
    if (elapsed > s_window_ms) {
        // 新窗�?        entry->request_count = 1;
        entry->window_start = now;
        xSemaphoreGive(s_mutex);
        return true;
    }

    // 窗口内计�?    entry->request_count++;

    if (entry->request_count > s_max_requests) {
        // 超限，阻�?        entry->blocked = true;
