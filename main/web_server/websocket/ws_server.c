...
/**
 * @brief WebSocket 服务器结�? */
struct ws_server {
    httpd_handle_t http_server;     // HTTP 服务器句�?    ws_client_mgr_t client_mgr;     // 客户端管理器
...
// 全局服务器句柄（�?handler 使用�?static ws_server_t *s_global_server = NULL;
...
esp_err_t ws_uri_handler(httpd_req_t *req)
{
    ...
    if (req->method == HTTP_GET) {
        ...
        // 添加客户�?        int idx = ws_client_mgr_add(&server->client_mgr, fd, "unknown");
        if (idx < 0) {
    ...
    // �?GET 请求 = 接收 WebSocket �?    int fd = httpd_req_to_sockfd(req);
    ...
    switch (ws_pkt.type) {
        ...
        case HTTPD_WS_TYPE_CLOSE: {
            ...
            // 回复 close �?            httpd_ws_frame_t close_pkt = {
                .type = HTTPD_WS_TYPE_CLOSE,
                .payload = NULL,
                .len = 0,
            };
        ...
        case HTTPD_WS_TYPE_PONG: {
            // Pong 响应，更新活动时�?            ws_client_mgr_update_activity(&server->client_mgr, fd);
...
ws_server_t* ws_server_create(httpd_handle_t http_server, const ws_server_config_t *config)
{
    ...
    // 初始化客户端管理�?    ws_client_mgr_config_t mgr_config = {
...
void ws_server_destroy(ws_server_t *server)
{
    ...
    // 销毁客户端管理�?    ws_client_mgr_deinit(&server->client_mgr);
...
void ws_server_dump_status(ws_server_t *server)
{
    ...
    if (server->heartbeat_enabled) {
        ...
        ESP_LOGI(TAG, "Heartbeat stats: pings_sent=%u, timeouts=%u, pongs_received=%u",
                 pings, timeouts, pongs);
    }

    ESP_LOGI(TAG, "Server stats: msg_sent=%u, msg_failed=%u",
